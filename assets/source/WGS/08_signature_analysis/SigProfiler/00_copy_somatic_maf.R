...
destination.dir <- paste(run.dir, "/result/SigProfiler/", study.name, "_signature_analysis", sep="")
...
message( paste("Copying somatic variant maf files for ", study.name, sep=""))
	...
	FUN = function( curr.sample.id ){
		...
		file.copy(from=curr.maf, to=paste(destination.dir, "/", curr.sample.id, ".maf", sep=""), copy.date=TRUE )
	},
	mc.cores = threads)
