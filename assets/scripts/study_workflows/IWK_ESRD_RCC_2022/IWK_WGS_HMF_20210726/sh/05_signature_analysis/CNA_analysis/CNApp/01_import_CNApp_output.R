...
cna_freqs.dt.ls <- lapply(
	...
	FUN = function(curr.annotation.type){
			...
			curr.file <- system( paste('ls ', curr.dir, '/cna_frequencies*.tsv', sep=""), intern=TRUE)
...
cna_freqs.dt[,chrom:=strsplit(regions, split='_', fixed=TRUE)[[1]][[1]], by=1:nrow(cna_freqs.dt)]
